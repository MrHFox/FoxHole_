...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        {
            // First part of help message is specific to FoxCoin server / RPC client
            std::string strUsage = _("FoxCoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  FoxCoin [options]                     " + "\n" +
                  "  FoxCoin [options] <command> [params]  " + _("Send command to -server or FoxCoin") + "\n" +
                  "  FoxCoin [options] help                " + _("List commands") + "\n" +
                  "  FoxCoin [options] help <command>      " + _("Get help for a command") + "\n";
        ...
        for (int i = 1; i < argc; i++)
            if (!IsSwitchChar(argv[i][0]) && !boost::algorithm::istarts_with(argv[i], "FoxCoin:"))
                fCommandLine = true;
...
{
    uiInterface.ThreadSafeMessageBox(str, _("FoxCoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("FoxCoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        "  -conf=<file>           " + _("Specify configuration file (default: foxcoin.conf)") + "\n" +
        "  -pid=<file>            " + _("Specify pid file (default: foxcoin.pid)") + "\n" +
        ...
        "  -port=<port>           " + _("Listen for connections on <port> (default: 9929 or testnet: 9939)") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9919)") + "\n" +
...
//    }
    ...
    // Make sure only a single FoxCoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s.  FoxCoin is probably already running."), GetDataDir().string().c_str()));
    ...
    if (fDaemon)
        fprintf(stdout, "FoxCoin server starting\n");
    ...
    // requested to kill FoxCoin-qt during the last operation. If so, exit.
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        if (nLoadWalletRet == DB_CORRUPT)
            ...
            strErrors << _("Error loading foxhole.dat: Wallet requires newer version of FoxCoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("FoxHole needed to be rewritten: restart FoxCoin to complete") << "\n";
