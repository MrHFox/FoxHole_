...
// Global state
//
static int DIFFICULTYKIMOTOFORKHEIGHT = 4;
...
uint256 hashGenesisBlock("0x351c6703813172725c6d660aa539ee6a3d7a9fe784c87fae7f36582e3b797058");
...
const string strMessageMagic = "FoxCoin Signed Message:\n";
...
static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // FoxCoin: 1 days
static const int64 nTargetSpacing = 60; // FoxCoin: 1 minute blocks
...
unsigned int static TrollKimotoGravityWell(const CBlockIndex* pindexLast, uint64 TargetBlocksSpacingSeconds, uint64 PastBlocksMin, uint64 PastBlocksMax) {
...
unsigned int TrollNeoGetNextWorkRequired(const CBlockIndex* pindexLast)
{
    ...
    return TrollKimotoGravityWell(pindexLast, BlocksTargetSpacing, PastBlocksMin, PastBlocksMax);
}
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal FoxCoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "FoxCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x8aa240aa215f8bc1661ca27f093bade0573c042fa3da49033d2f6378c4331047");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Rickrolled.... IN F**KING 2014!!!!!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1390207760; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 3175355;

        if (fTestNet)
        {
            block.nTime    = 1389937920;
            block.nNonce   = 2846695;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x67ab2ad44f76e60cef08fccf9e26c0a65aaec508e5b7c75df2591723e4d11ecd"));
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // Litecoind: Reduce the exempted free transactions to 500 bytes (from Foxcoin's 3000 bytes)
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("FoxcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(NULL, pblock))
            return error("FoxcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ThreadFoxcoinMiner(void* parg);

static bool fGenerateFoxcoins = false;
static bool fLimitProcessors = false;
static int nLimitProcessors = -1;

void static FoxcoinMiner(CWallet *pwallet)
{
    printf("FoxcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    // Make this thread recognisable as the mining thread
    RenameThread("foxcoin-miner");

    // Each thread has its own key and counter
    CReserveKey reservekey(pwallet);
    unsigned int nExtraNonce = 0;

    while (fGenerateFoxcoins)
    {
        if (fShutdown)
            return;
        ...
        printf("Running FoxcoinMiner with %d transactions in block\n", pblock->vtx.size());
...
void static ThreadFoxcoinMiner(void* parg)
{
    ...
    try
    {
        ...
        FoxcoinMiner(pwallet);
        vnThreadsRunning[THREAD_MINER]--;
    }
    catch (std::exception& e) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(&e, "ThreadFoxcoinMiner()");
    } catch (...) {
        vnThreadsRunning[THREAD_MINER]--;
        PrintException(NULL, "ThreadFoxcoinMiner()");
    }
    nHPSTimerStart = 0;
    if (vnThreadsRunning[THREAD_MINER] == 0)
        dHashesPerSec = 0;
    printf("ThreadFoxcoinMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_MINER]);
}


void GenerateFoxcoins(bool fGenerate, CWallet* pwallet)
{
    fGenerateFoxcoins = fGenerate;
    nLimitProcessors = GetArg("-genproclimit", -1);
    if (nLimitProcessors == 0)
        fGenerateFoxcoins = false;
    ...
    if (fGenerate)
    {
        ...
        printf("Starting %d FoxcoinMiner threads\n", nAddThreads);
        for (int i = 0; i < nAddThreads; i++)
        {
            if (!CreateThread(ThreadFoxcoinMiner, pwallet))
                printf("Error: CreateThread(ThreadFoxcoinMiner) failed\n");
            Sleep(10);
        }
    }
}
